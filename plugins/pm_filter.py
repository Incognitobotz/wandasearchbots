        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>Searching For <code>{is_misspelled}</code></i></b>")
    ...
    if save == True and SAVE_RESTRICTED_MODE == True:
        try:
            ...
            if not link:
                ...
                if PM_SEARCH == True:
                    ai_search = True
                    reply_msg = await bot.send_message(message.from_user.id, f"<b><i>Searching For <code>{is_misspelled}</code></i></b>", reply_to_message_id=message.id)
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>Searching For <code>{is_misspelled}</code></i></b>")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # don't change anything without contacting me @kingvj01
        ...
        buttons = [[
            InlineKeyboardButton('🌿 ꜱᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url=SUPPORT_CHAT),
            InlineKeyboardButton('🖥️ Download 🖥️', url=TUTORIAL)
        ],[
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/BarkesyaXboy")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    try:
        ...
        k = await reply_msg.edit_text(text=script.MVE_NT_FND.format(mv_rqst), reply_markup=InlineKeyboardMarkup(button))
